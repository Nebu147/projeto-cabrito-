...
def baseline_fix(pt: float) -> float:
    ...
    # 0.28~0.32 funciona bem para Helvetica
    return 0.30 * pt

def compute_col_r(coords, sx, page_w):
    # Se houver Xs nos valores da direita, usa o MAIOR após escala; senão usa 0.89*page_w
    xs = []
    for k in RIGHT_KEYS:
        if k in coords:
    ...
    if xs:
        # ancora um pouco à direita do maior x visto
        return max(xs)
    return 0.89 * page_w

def draw_text(c: canvas.Canvas, text, x, y, font, pt, align="left"):
    try:
        c.setFont(font, pt)
    except:
...
def main():
    ...
    bg = Path(args.bg); coords_path = Path(args.coords); out = Path(args.out)
    if not bg.exists(): raise FileNotFoundError(f"Fundo não encontrado: {bg}")
    if not coords_path.exists(): raise FileNotFoundError(f"Coords não encontrado: {coords_path}")
    ...
    for name, meta in coords.items():
        ...
        # Prova visual
        if args.proof:
            txt_w = pdfmetrics.stringWidth(text, font if font in pdfmetrics.getRegisteredFontNames() else "Helvetica", pt)
            pad = 2
            if name in RIGHT_KEYS:
                x_box = col_r - (txt_w if align=="right" else txt_w/2 if align=="center" else 0)
            else:
                x_box = X if align=="left" else (X - txt_w/2 if align=="center" else X - txt_w)
            c.setDash(2,2); c.rect(x_box - pad, Y - pad, txt_w + 2*pad, pt + 2*pad, stroke=1, fill=0); c.setDash()
